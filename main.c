...
void svm_verbose(const float *dataset, const float *alpha, float beta);
void init_kernel(float *kernel, const float *dataset);
void init_weight(float *weight, const float *dataset, const float *alpha);
int predict(const float *sample, const float *weight, float beta, int feature_count);
float y_pred(const float *dataset, const float *kernel, const float *alpha, float beta, int sample_index);
void SMO(const float *dataset, const float *kernel, float C, float tol, int max_passes, float *alpha, float *beta, int verbose);
...
int main(int argc, char **argv)
{
    ...
    // Produce the kernel, run the smo algorithm, init weights based on results
    
    init_kernel(kernel, dataset);
    SMO(dataset, kernel, C, tolerance, max_iter, alpha, &beta, 1);
    init_weight(weight, dataset, alpha);
...
void dataset_verbose(const float *dataset)
{
    ...
    for(i = 0; i < sample_count(dataset); i++){
        const float *sample = get_sample(dataset, i);
...
void svm_verbose(const float *dataset, const float *alpha, float beta){
    float weight[2];
    init_weight(weight, dataset, alpha);
    printf("weight: (%.2f, %.2f) \t beta: %.2f\n", weight[0], weight[1], beta);
}
        ...
        for(i = 0; i < sample_size; i++){
            ...
            if((yi * err_i < -tol && alpha[i] < C) || (yi * err_i > tol && alpha[i] > 0)){
                ...
                *beta = calc_b(alpha[i], alpha[j], b1, b2, C);
                ...
                if(verbose) svm_verbose(dataset, alpha, *beta);
